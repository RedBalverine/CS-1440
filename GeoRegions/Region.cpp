...
unsigned int Region::computeTotalPopulation()
{
    ...
    if(m_totalPopulation == 0) {
        for (int i = 0; i < m_region.size(); i++) {
            ...
            m_totalPopulation += m_region[i]->getPopulation();
        }
        return m_totalPopulation + m_population;
    }
}
...
void Region::list(std::ostream& out)
{
    ...
    for(int i = 0; i < m_region.size(); i++)
    {
        out << m_region[i]->m_id << " : " << m_region[i]->m_name << std::endl;
    }
...
void Region::display(std::ostream& out, unsigned int displayLevel, bool showChild)
{
    ...
    unsigned int totalPopulation = computeTotalPopulation();
    double area = getArea();
...
//    for(int i  = 0; i < m_SubRegionCount)
    // foreach subregion,
    //      save that region
...
Region* Region::getRegionById(const unsigned int id)
{
    for(int i = 0; i < m_region.size(); i++)
    {
        if (m_region[i]->getId() == id)
        {
            return m_region[i];
        }
    }
...
void Region::remove(int id)
{
    for(int i = 0; i < m_region.size(); i++)
    {
        if (m_region[i]->getId() == id)
        {
            delete m_region[i];
            m_region[i] = nullptr;
            m_region.erase(m_region.begin() + i);
        }
    }
