...
void RegionTester::testSubRegions()
{
    ...
    //test to add subregion
    //test count
    Region* regionTest;
    Region* regionTest2;
    regionTest->setName("Ashlee");
    regionTest2->addSubRegion(regionTest);

    if(regionTest2->getSubRegionByIndex(0)->getName() != regionTest->getName())
    {
        std::cout << "Failed to add the Nation of Korea" << std::endl;
    }

    Region* regionTest3;
    regionTest3->setName("Korea");
    regionTest2->addSubRegion(regionTest3);

    if(regionTest2->getSubRegionCount() != 2)
    {
...
void RegionTester::testComputeTotalPopulation()
{
    ...
    //add subregion's population together and test

    Region* region;
    region->setPopulation(50);
    Region* region2;
    region2->setPopulation(150);
    region->addSubRegion(region2);

    if(region->computeTotalPopulation() != 200)
    {
        std::cout << "Failed to compute Total Population" << std::endl;
    }

    Region* region3;
    region3->setPopulation(200);
    region->addSubRegion(region3);

    if(region->computeTotalPopulation() != 400)
    {
        std::cout << "Failed to compute Total Population" << std::endl;
